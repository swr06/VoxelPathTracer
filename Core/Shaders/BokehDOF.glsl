// Bokeh depth of field with support for smooth chromatic aberration
...
void main() {
	...
	float DelinearizedCenterDepth = DelinearizeDepth(Depth);
	...
	for (int Sample = 0 ; Sample < (u_LargeKernel ? 50 : 30) ; Sample++) {
		
		float Weight = 1.0f;

		vec2 BokehOffset = u_LargeKernel ? BokehOffsets50[Sample] : BokehOffsets30[Sample];
