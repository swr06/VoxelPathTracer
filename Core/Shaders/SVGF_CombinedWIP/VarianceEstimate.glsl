...
layout (location = 3) out float o_SpecularVariance;
    ...
    if (SPP < SPP_THRESH)
    {
        for (int x = -2 ; x <= 2 ; x++) 
        {
            for (int y = -2 ; y <= 2 ; y++)
            {
                ...
                vec4 SampleSpec = texture(u_SpecularSH, v_TexCoords + vec2(x,y)*TexelSizeSpecSH).xyzw;
                float Sl = SHToY(SampleSpec);
        ...
        o_SpecularVariance = TotalSpecularVariance;
    } 
